...
---            registered   → send HEARTBEAT directly to controlplane_id.
...
--- @param opts         table   { node, interval_seconds?, miss_limit?, label?, labels? }
---                             node is required — enforced contract.
--- @return NodeAgent
function NodeAgent.new(ctx, computer_id, transport, opts)
  ...
  assert(type(opts.node) == "string" and opts.node ~= "", "opts.node must be a non-empty string")
...
function NodeAgent:step()
  ...
  if not self._registered then
    ...
    if ack and ack.type == "ACK" then
      self._controlplane_id = ack.master_id
      self._registered      = true
      self._miss_count      = 0
    end
  else
    self._transport.send(self._controlplane_id, { type = "HEARTBEAT", id = self._id })
    local _, ack = self._transport.receive(ACK_TIMEOUT)
    if ack and ack.type == "ACK" then
      self._miss_count = 0
    else
